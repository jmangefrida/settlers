...
class Map(object):
    ...
    def build_mini(self):
        mdirt = b'\xcc\x66\x00\xAA'
        mtree = b'\x00\x99\x00\xAA'
        more = b'\x99\x99\x66\xCC'
        mwater = b'\x00\x66\xff\xCC'
        mini = b''
        for y in range(0, self.height):
            for x in range(0, self.width):
                if self.orem[x, y] > 0:
...
class Person(object):
    ...
    def do_task(self):
         ...
         try:
             step = self.task_list[0][1][-1]
             print(step)
         except:
             print('badstep')
             print (self.task_list)
         self.task_list[0][1] = self.task_list[0][1][:-1]  # remove 1 item from the list of steps
...
class Explorer(Job):
    ...
    def scan(self):
        super(Explorer, self).scan(self.person.map.wood)

    def wander(self):
        x = randint(0, self.person.map.width * 40)
        x = int(x / 10)
        x = x * 10
        y = randint(0, self.person.map.height* 40)
        y = int(y / 10)
        y = y * 10
        self.move(x,y)
    ...
    def move(self, x, y):
        steps = [[x, y]]
        while x != self.person.x or y != self.person.y:
            steps.append([x,y])
            if x < self.person.x:
                x = x + 10
            elif x > self.person.x:
                x = x - 10
            if y < self.person.y:
                y = y + 10
            elif y > self.person.y:
                y = y - 10
...
for z in range(100):
    person = Person(map, character, randint(0, map.width) * 10 , randint(0, map.height) * 10)
...
while True:
    ...
    -trans_x / map.tile_size, -trans_y / map.tile_size, window_x / map.tile_size, window_y / map.tile_size), 1)
    pygame.display.flip()
    # print('flip')
